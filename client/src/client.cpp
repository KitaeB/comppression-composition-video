...
void lz4_noconcat_noprime(tcp::socket &socket) {
    ...
    while (true)
    {
        ...
        if (lz4_decompress(compressed_data_frame_1, uncompressed_data_frame_1, uncompressed_size))
        {
            frame1 = convertFromCleanDataChar(uncompressed_data_frame_1, frame1.rows, frame1.cols, frame1.type());
            cv::imshow("webcam1", frame1);
        }
        
        frame2 = cv::Mat(rows, cols, type);
        if (lz4_decompress(compressed_data_frame_2, uncompressed_data_frame_2, uncompressed_size))
        {
            frame2 = convertFromCleanDataChar(uncompressed_data_frame_2, frame2.rows, frame2.cols, frame2.type());
